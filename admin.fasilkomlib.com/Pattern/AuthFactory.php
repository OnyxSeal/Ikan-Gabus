...
class AdminRegisterHandler implements AuthInterface {
    ...
    public function handle($data) {
        ...
        $nohp = $data['nohp'];
        $position = $data['position'] ?? 'admin'; // nilai default "admin"
        ...
        if (mysqli_num_rows($check_user_result) > 0) {
            ...
            $insert_query = "INSERT INTO admin (profile_picture, fullname, email, usradm, pwadm, nohpadm, position) 
                            VALUES ('$avatar', '$fullname', '$email', '$username', '$password', '$nohp', '$position')";
